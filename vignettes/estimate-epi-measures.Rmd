...
This document shows convenient functions from `injurytools` to describe sports injury data, in terms of measures used in sports injury epidemiology (see @Bahr2003). These measures are explained below and then [`injsummary()`](https://lzumeta.github.io/injurytools/) and [`injprop()`](https://lzumeta.github.io/injurytools/) functions are illustrated.
...
*NOTE 1:* as @Bahr2018 state, injury incidence (likelihood) and injury burden (severity) should be reported and assessed in conjunction rather than in isolation. In this regard, see the risk matrix plot provided by [`gg_injriskmatrix()`](https://lzumeta.github.io/injurytools/) function. 
...
In the near future there will be available the negative binomial (`method = "negbin"` argument), zero-inflated poisson ("`zinfpois`") and zero-inflated negative binomial (`"zinfnb"`) methods in [`injsummary()`](https://lzumeta.github.io/injurytools/) function.
