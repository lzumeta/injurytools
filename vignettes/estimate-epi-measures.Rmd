...
Ǹow, the preprocessed data is passed to `injsummary()` to calculate injury summary statistics:

```{r}
injds <- injsummary(injd)
```

We notice that it throws some warning messages (unless `quiet = TRUE`). They are thrown to make it clear what the exposure time unit is[^estimate-note-1]. 

[^estimate-note-1]: Something that it is important to be aware of.
...
```{r}
head(availability_table)
```

```{r}
availability_table %>% 
  filter(type_injury == "Muscle")
```
