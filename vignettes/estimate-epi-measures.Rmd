...
Whenever one collects and prepares the data, the next natural step is to summarize and explore these data. In this case, from a sports-applied point of view, one wants to know how many and what type of injuries have occurred, how often they have occurred or what the load has been.
...
$$P = \frac{X}{N}$$
where $X$ is the number of injury cases and $N$ the total number of players in the study at any point in the window of time $\Delta T$.
...
```{r}
# format the table of playerwise dataframe
injds[[1]] %>% 
